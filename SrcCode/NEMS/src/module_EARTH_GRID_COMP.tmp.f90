...
!
     ...
     if (.not.NUOPC_FieldDictionaryHasEntry( &
      ...
      if (.not.NUOPC_FieldDictionaryHasEntry( &
        "eastward_sea_state_dependent_fric_vel")) then
        call NUOPC_FieldDictionaryAddEntry( &
          standardName="eastward_sea_state_dependent_fric_vel", &
          canonicalUnits="m s-1", &
          rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=2852, &
      ...
      if (.not.NUOPC_FieldDictionaryHasEntry( &
        "northward_sea_state_dependent_fric_vel")) then
        call NUOPC_FieldDictionaryAddEntry( &
          standardName="northward_sea_state_dependent_fric_vel", &
          canonicalUnits="m s-1", &
          rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=2864, &
...
!
      ...
      subroutine SetModelServices(driver, rc)
        ...
        if (ESMF_LogFoundAllocError(statusToCheck=stat, &
          ...
          line=3390, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90", rcToReturn=rc)) &
          return  ! bail out
        ...
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=3399, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
        
        ! read and ingest free format driver attributes
        attrFF = NUOPC_FreeFormatCreate(config, label="EARTH_attributes::", &
          relaxedflag=.true., rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=3405, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
        call NUOPC_CompAttributeIngest(driver, attrFF, addFlag=.true., rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=3408, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
        ...
        if (trim(value)=="true") then
          ...
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=3430, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
          call ESMF_LogWrite( &
            "===>===>===>===> Done Dumping Field Dictionary <===<===<===<===", &
            ESMF_LOGMSG_INFO, rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=3435, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
        endif
        ...
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=3442, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
        allocate(compLabels(componentCount), stat=stat)
        if (ESMF_LogFoundAllocError(statusToCheck=stat, &
          msg="Allocation of compLabels failed.", &
          line=3446, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90", rcToReturn=rc)) &
          return  ! bail out
        ...
        do i=1, componentCount
          ...
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=3492, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
          ...
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=3502, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
          ...
          if (trim(model) == "none") then
            ...
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3507, &
          ...
          if (trim(model) == "satm") then
            ...
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3526, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "xatm") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3539, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "datawam") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3552, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "gsm") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3565, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "nmmb") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3578, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "fv3") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3591, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "datm") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3604, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "socn") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3617, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "xocn") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3630, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "hycom") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3643, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "mom5") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3656, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "mom6") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3669, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "pom") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3682, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "sice") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3695, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "xice") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3708, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "cice") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3721, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "swav") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3734, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "xwav") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3747, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          ...
          elseif (trim(model) == "slnd") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3773, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "xlnd") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3786, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "noah") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3799, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "lis") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3812, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "sipm") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3825, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "xipm") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3838, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "ipe") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3851, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "dataipe") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3864, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "shyd") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3877, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "xhyd") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3890, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc) 
            return  ! bail out
          elseif (trim(model) == "wrfhydro") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3903, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "gsdchem") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3916, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          elseif (trim(model) == "adcirc") then
            ...
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
              line=3925, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  !  bail out
          elseif (trim(model) == "hwrfdata") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3942, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          ...
          elseif (trim(model) == "atmesh") then
            call NUOPC_DriverAddComp(driver, trim(prefix), ATMESH_SS, &
              petList=petList, comp=comp, rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
              line=3964, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  !  bail out
          elseif (trim(model) == "nwm") then
            write (msg, *) "Model '", trim(model), "' was requested, "// &
              "but is not available in the executable!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=3981, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
! - Two mediator choices currently built into NEMS from internal
          elseif (trim(model) == "nems") then
            call NUOPC_DriverAddComp(driver, trim(prefix), MED_SS, &
              petList=petList, comp=comp, rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
              line=3993, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
          elseif (trim(model) == "spaceweather") then
            call NUOPC_DriverAddComp(driver, trim(prefix), MEDSW_SS, &
              petList=petList, comp=comp, rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
             line=3998, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
          else
            ! Error condition: unknown model requested
            write (msg, *) "The requested model '", trim(model), &
              "' is an invalid choice!"
            call ESMF_LogSetError(ESMF_RC_NOT_VALID, msg=msg, line=4003, &
              file="module_EARTH_GRID_COMP.F90", rcToReturn=rc)
            return  ! bail out
          endif
          ...
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=4012, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
          call NUOPC_CompAttributeIngest(comp, attrFF, addFlag=.true., rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=4015, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
          call NUOPC_FreeFormatDestroy(attrFF, rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=4018, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
  ...
  subroutine SetRunSequence(driver, rc)
    ...
    call ESMF_GridCompGet(driver, name=name, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=4066, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out

    ! read free format run sequence from config
    call ESMF_GridCompGet(driver, config=config, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=4072, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
    runSeqFF = NUOPC_FreeFormatCreate(config, label="runSeq::", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=4075, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out

    ! ingest FreeFormat run sequence
    call NUOPC_DriverIngestRunSequence(driver, runSeqFF, &
      autoAddConnectors=.true., rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=4081, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
    
    ! Diagnostic output
    if(verbose_diagnostics()) then
       call NUOPC_DriverPrint(driver, orderflag=.true., rc=rc)
       if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=4093, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
    endif
  ...
  subroutine Finalize(driver, rc)
    ...
    call ESMF_GridCompGet(driver, name=name, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=4392, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
    
    ! query Component for this internal State
    nullify(is%EARTH_INT_STATE)
    call ESMF_GridCompGetInternalState(driver, is, rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=4398, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90")) return  ! bail out
      
    ! deallocate internal state memory
    deallocate(is%EARTH_INT_STATE, stat=stat)
    if (ESMF_LogFoundDeallocError(statusToCheck=stat, &
      msg="Deallocation of internal state memory failed.", &
      line=4404, file=trim(name)//":"//"module_EARTH_GRID_COMP.F90", rcToReturn=rc)) &
      return  ! bail out
