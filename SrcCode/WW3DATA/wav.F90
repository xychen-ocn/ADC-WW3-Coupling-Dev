...
module WAV
  ...
  use wav_mod, only: SXX, SYY, SXY,SSDWSX, SSDWSY
...
!  end subroutine
  ...
  subroutine ModelAdvance(model, rc)
    ...
    !----------------------------------------
!
! XYC: add SSD wind stress pack and send from here 
    !---------------------------------
    ! >>>>> PACK and send SSDWSX
    call State_getFldPtr(ST=exportState,fldname='ssdwsx',fldptr=dataPtr_ssdwsx,rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    !fill only owned nodes for tmp vector
    do i1 = 1, mdataOutw%NumOwnedNd, 1
        tmp(i1) = SSDWSX(mdataOutw%owned_to_present_nodes(i1),1)
    end do
    !assign to field
    dataPtr_ssdwsx = tmp
    ...
    call State_getFldPtr(ST=exportState,fldname='ssdwsy',fldptr=dataPtr_ssdwsy,rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    !fill only owned nodes for tmp vector
    do i1 = 1, mdataOutw%NumOwnedNd, 1
        tmp(i1) = SSDWSY(mdataOutw%owned_to_present_nodes(i1),1)
    end do
    !assign to field
    dataPtr_ssdwsy = tmp
