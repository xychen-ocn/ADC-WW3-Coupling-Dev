...
module adc_cap
  ...
  ! XYC add wind stress component:
  USE GLOBAL,  ONLY: WVNSX1, WVNSY1
  USE GLOBAL,  ONLY: WVNSX2, WVNSY2
  ! XYC finished adding.
  ...
  use GLOBAL,  ONLY: RhoWat0, NUOPC4MET, NUOPC4WAV, NWS, g, DUMY1
...
!
    ...
    call fld_list_add(num=fldsToAdc_num, fldlist=fldsToAdc,stdname="eastward_sea_state_dependent_fric_vel", shortname="ssdwsx")
    call fld_list_add(num=fldsToAdc_num, fldlist=fldsToAdc,stdname="northward_sea_state_dependent_fric_vel", shortname="ssdwsy")
...
!m s-1	Ocean current Y component.	 	
  ...
  subroutine fld_list_add(num, fldlist, stdname, data, shortname, unit)
    ...
    write(info,*) subname,' --- Passed--- ', fldlist(num)%stdname
    !print *,      subname,' --- Passed --- '
...
!  end subroutine
  ...
  subroutine ModelAdvance(model, rc)
    ...
    ! XYC add: SSD wind stress from the WW3:
    real(ESMF_KIND_R8), pointer:: dataPtr_ssdwsx(:)
    real(ESMF_KIND_R8), pointer:: dataPtr_ssdwsy(:)
    ...
    call setMessageSource(trim(subname))    ! XYC added in debugging
                                           !  see global.F
    call allMessage(1,info)
    call unsetMessageSource()               ! XYC added see global.F
   ...
   do num = 1,fldsToAdc_num
      ...
      ! XYC: add sea-state-dependent wind stress here: (check if variables are
      ! connected?)
      if (fldsToAdc(num)%shortname == 'ssdwsx') wave_forcing = wave_forcing .and. fldsToAdc(num)%connected
      if (fldsToAdc(num)%shortname == 'ssdwsy') wave_forcing = wave_forcing .and. fldsToAdc(num)%connected
   end do
    ...
    if (wave_forcing) then
        ...
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          ...
          return  ! bail out
        !-----------------------------------------
        ! <<<<< RECEIVE and UN-PACK SSDWSX (XYC added)
        call State_getFldPtr(ST=importState,fldname='ssdwsx',fldptr=dataPtr_ssdwsx,rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
          return  ! bail out
        !-----------------------------------------
        ! <<<<< RECEIVE and UN-PACK SSDWSY (XYC added)
        call State_getFldPtr(ST=importState,fldname='ssdwsy',fldptr=dataPtr_ssdwsy,rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
          return  ! bail out
        ...
        print *, 'size dataPtr_ssdwsx > ', size(dataPtr_ssdwsx)
...
!    unit will be m2s-2 which is the correct one.   
        ...
        ! -------------- XYC added SSD wind stresses following the wave radiation stress ---------
        ! Allocate arrays for SSD wind stresses:
        ! XYC add: wind forcing to here. 
        !   <<<< Receive and unpack ssdwsx, ssdwsy (sea-state dependent wind stress
        !   components)
        
        IF(.NOT.ALLOCATED(WVNSX1)) ALLOCATE(WVNSX1(1:NP))
        IF(.NOT.ALLOCATED(WVNSY1)) ALLOCATE(WVNSY1(1:NP))
        
        IF(.NOT.ALLOCATED(WVNSX2)) ALLOCATE(WVNSX2(1:NP))
        IF(.NOT.ALLOCATED(WVNSY2)) ALLOCATE(WVNSY2(1:NP))
        ...
        ! Ghost nodes update
        call UPDATER( WVNSX1(:), WVNSY1(:),DUMY1, 2)
        call UPDATER( WVNSX2(:), WVNSY2(:),DUMY1, 2)
        !write(*,*) "[debug:nws18 updater:] max(WVNSX/Y1)=", maxval(WVNSX1), maxval(WVNSY1)
        !write(*,*) "[debug:nws18 updater:] max(WVNSX/Y2)=", maxval(WVNSX2), maxval(WVNSY2)
!        if (sum(WVNSX2) .eq. 0.0 ) then
!          WVNSX2 = -1.0
!        end if  
!        if (sum(WVNSY2) .eq. 0.0 ) then
!          WVNSY2 = -1.0
!        end if  
      
       
        ! XYC note: an extra block to constrain the magnitude of the wind stress
        ! at first exchange may be needed as in the wind speed case.
...
!!!!#ifdef NO_COMPILE00000
   ...
   !     if ((currTime > BeforeCaribbeanTime) .and. (currTime < AfterCaribbeanTime)) then
            ...
            write(info,*) subname, 'in cap after maxval(WVNSX2)', maxval(WVNSX2)
            call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=dbrc)

            write(info,*) subname, 'in cap after maxval(WVNSY2)', maxval(WVNSY2)
            call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=dbrc)
